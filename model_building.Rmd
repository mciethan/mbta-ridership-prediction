...
model_data_f23 <- get_dataset('f23') %>%
  inner_join(get_dataset('f23_geog_vars'), by=c('route_id', 'parent_station')) %>%
  inner_join(get_dataset('f23_bus&CR_connections'), by=c('route_id', 'parent_station')) %>%
...
```{r}
c('avg_boardings', 'avg_headway', 'avg_tt', 'walk_score', 'km_from_cbd',  
  'avg_median_inc', 'ppsqft', 'jobspersqft', 'devl_mix') %>% 
...
```{r}
basic_reg <- lm(avg_boardings ~ walk_score + pnr_spaces + connecting_cr_routes
                + connecting_bus_riders + avg_headway + avg_tt + transfer
                + Red + Orange + Blue + km_from_cbd
                + avg_median_inc #+ tot_jobs + tot_pop 
...
plot_y_yhat_w_labels <- function(x, y, xy_prefix) {
  ...
  text(x=x, y=y, adj=c(0.5,1), cex=0.5,
       labels=paste0(
         str_sub(model_data_f23$route_id, end=1), 
         '-', str_sub(model_data_f23$stop_name, end=3))
       )
}

plot_y_yhat_w_labels(model_data_f23$avg_boardings, predict(basic_reg), '')
plot_y_yhat_w_labels(log(model_data_f23$avg_boardings), log(predict(basic_reg)), 'Logged')
```
