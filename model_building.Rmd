...
get_model_data <- function(season) {
  get_dataset(season) %>%
    ...
    inner_join(get_dataset(paste(season, 'spacings', sep='_')), by=c('route_id', 'parent_station')) %>%
    ...
    inner_join(get_dataset('acs2022_5yr_vehicles_available'), by=c('route_id', 'parent_station')) %>%
    ...
    left_join(get_dataset(paste(season, 'terminals', sep='_')), by=c('route_id', 'parent_station')) %>%
...
```{r}
continuous_vars <- c('avg_boardings', 'avg_headway_wkdy', 'avg_headway_wknd', 'avg_trav_time_to_cbd', 
                     'avg_spacing_km', 'walk_score', 'km_from_cbd', 'median_inc_1000s', 'pop_per_acre',
                     'jobs_per_acre', 'devl_mix', 'hh_no_vehicle_share', 'avg_veh_per_hh')
...
summaries[['Fall 2023 weekday base']] # display summary for a single specification
```
