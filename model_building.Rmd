...
base_reg <- function(model_data) {
  lm(avg_boardings ~ avg_headway + Red + Orange + Blue + transfer
...
summaries[['Fall 2023 enhanced']] # display summary for a single specification
regressions %>% lapply(vif) # test for multicollinearity across models
```
...
var_names <- coefficients[['Fall 2023 enhanced']] %>% row.names() 
...
```{r}
formatted_output %>% write.csv('formatted_output.csv')
```
...
specifications[['Fall 2023']] %>%
  plot_y_yhat_w_labels(predict(regressions[['Fall 2023 enhanced']]), '')

specifications[['Fall 2023']] %>%
  mutate(avg_boardings = log(avg_boardings)) %>%
  plot_y_yhat_w_labels(predict(regressions[['Fall 2023 enhanced']]) %>% log(), 'Logged')
```
