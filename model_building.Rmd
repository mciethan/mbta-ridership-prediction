...
continuous_vars <- c('avg_boardings', 'avg_headway_wkdy', 'avg_headway_wknd', 
                     ...
                     'devl_mix', 'hh_no_vehicle_share', 'avg_veh_per_hh')
...
base_reg <- function(params) {
  eval(bquote(
    lm(.(as.symbol(paste0('avg_boardings_', params$daytype))) ~ avg_trav_time_to_cbd + #cov_tt +
       avg_headway + avg_spacing_km
       + Red + Orange + Blue + transfer + terminal #+ km_from_cbd
       + connecting_cr_routes + connecting_bus_routes + pnr_spaces_100s
       + walk_score + median_inc_1000s + avg_veh_per_hh + pop_per_acre + jobs_per_acre 
       , data=params$data %>% 
...
specifications <- list(
  ...
  "Fall 2019 weekday (Boston only)" = list(data=model_data_f19 %>% filter(in_bos == 1), daytype='wkdy'),
  "Fall 2023 weekday (Boston only)" = list(data=model_data_f23 %>% filter(in_bos == 1), daytype='wkdy'),
  "Fall 2019 weekend" = list(data=model_data_f19, daytype='wknd'), 
  "Fall 2023 weekend" = list(data=model_data_f23, daytype='wknd'),
  "Fall 2019 weekend (Boston only)" = list(data=model_data_f19 %>% filter(in_bos == 1), daytype='wknd'),
  "Fall 2023 weekend (Boston only)" = list(data=model_data_f23 %>% filter(in_bos == 1), daytype='wknd')
)
...
summaries[['Fall 2023 weekday enhanced']] # display summary for a single specification
```
...
var_names <- coefficients[[length(coefficients)]] %>% row.names() # get variable names from last model
...
plot_y_yhat_w_labels <- function(df, reg, add_labels=FALSE, transformation=I, transform_prefix='') {
  ...
  if (add_labels) {
    df_large_resid <- df[abs(resid) > 2500,]
    y_large_resid <- df_large_resid[[yname]] %>% transformation()
    yhat_large_resid <- yhat[abs(resid) > 2500] #%>% transformation()
    text(x=y_large_resid, y=yhat_large_resid, adj=c(0.5,1), cex=0.5,
       labels=paste0(
...
specifications[['Fall 2023 weekday']]$data %>%
  plot_y_yhat_w_labels(regressions[['Fall 2023 weekday enhanced']], add_labels=T)

specifications[['Fall 2023 weekday']]$data %>%
  plot_y_yhat_w_labels(
    regressions[['Fall 2023 weekday enhanced']], #add_labels=T, 
    transformation=log, transform_prefix='Logged ')
```

```{r}
specifications[['Fall 2023 weekend']]$data %>%
  plot_y_yhat_w_labels(regressions[['Fall 2023 weekend base']], add_labels=TRUE)

specifications[['Fall 2023 weekend']]$data %>%
  plot_y_yhat_w_labels(
    regressions[['Fall 2023 weekend base']], #add_labels=T, 
    transformation=log, transform_prefix='Logged ')
```
