...
## Calculate the number of bus routes and riders that connect to each station
...
```{r}
clean_bus_connections <- function(rt_stations_df, buses_df) {
  rt_stations_df %>%
    left_join(buses_df, by='parent_station') %>%
    mutate(connecting_bus_routes = case_when(
        is.na(bus_rte_count) ~ 0, .default=bus_rte_count
      ),
      connecting_bus_riders = case_when(
        is.na(bus_rte_riders) ~ 0, .default=bus_rte_riders
      )) %>%
    select(route_id, parent_station, connecting_bus_routes, connecting_bus_riders)
}

rt_stations_f23 %>% clean_bus_connections(f23_buses) %>%
  write.csv('data/cleaned/stations_routes_f23_bus_connections.csv', row.names=FALSE)

rt_stations_f19 %>% clean_bus_connections(f19_buses) %>%
  write.csv('data/cleaned/stations_routes_f19_bus_connections.csv', row.names=FALSE)
```
...
```{r}
f23_q3_ttimes_hr <- read.csv(paste0(downloaded_traveltimes_path, '\\2023-Q3_HRTravelTimes.csv'))
f23_q3_ttimes_lr <- read.csv(paste0(downloaded_traveltimes_path, '\\2023-Q3_LRTravelTimes.csv'))
f23_q4_ttimes_hr <- read.csv(paste0(downloaded_traveltimes_path, '\\2023-Q4_HRTravelTimes.csv'))
f23_q4_ttimes_lr <- read.csv(paste0(downloaded_traveltimes_path, '\\2023-Q4_LRTravelTimes.csv'))
...
```{r}
f23_gtfs$calendar %>%
  inner_join(f23_gtfs$trips, by='service_id') %>%
  inner_join(f23_gtfs$routes, by='route_id') %>%
  filter(route_desc == 'Rapid Transit') %>%
  #group_by(start_date, end_date) %>%
  summarise(rating_start = min(start_date), rating_end = max(end_date))
```

Use the above to edit the date filters below if needed:

```{r}
f23_ttimes_rt <- f23_q3_ttimes_hr %>%
  rbind(f23_q3_ttimes_lr) %>%
  filter(service_date >= '2023-08-27') %>%
  rbind(
    f23_q4_ttimes_hr %>% rbind(f23_q4_ttimes_lr) %>% filter(service_date <= '2023-12-16')
  )
```

```{r}
rt_stops_f23 <- f23_gtfs$routes %>%
  filter(route_desc == 'Rapid Transit') %>%
  inner_join(f23_gtfs$trips, by='route_id') %>%
  inner_join(f23_gtfs$stop_times, by='trip_id') %>%
  inner_join(f23_gtfs$stops, by='stop_id') %>%
  select(stop_id, parent_station, stop_name) %>%
  unique() %>%
  mutate(stop_id = as.integer(stop_id)) 

ttime_metrics_f23 <- rt_stops %>%
  filter(stop_name %in% c('Park Street', 'Downtown Crossing', 'Government Center', 'State')) %>%
  inner_join(f23_ttimes_rt, by=join_by(stop_id == to_stop_id)) %>%
  mutate(route_id = case_when(str_detect(route_id, 'Green') ~ 'Green', .default=route_id)) %>%
  ...
  summarise(
    ...
    .groups='drop') %>%
  inner_join(rt_stops_f23, by=join_by(from_stop_id == stop_id)) %>%
  group_by(route_id, parent_station) %>% 
...
It also appears that this dataset does not record any travel times from Union Square (stop_id == 70504) to other places, which is odd. It does record travel times from other places to Union Square though, at inbound stop 70503. 
```{r}
rt_stops_f23 %>%
  filter(parent_station == 'place-unsqu') %>%
  inner_join(f23_ttimes_rt, by=join_by(stop_id == from_stop_id))
```

```{r}
ttime_metrics_f23 %>% write.csv('data/cleaned/stations_routes_f23_travel_times_to_cbd.csv')
```
...
```{r}
# this should have 0 rows. Any rows represent stations that didn't join to the walk scores
rt_stations_f23 %>%
