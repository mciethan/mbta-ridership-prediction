...
clean_ridership <- function(ridership) {
  riders_day_type <- ridership %>% 
    ...
    summarise( # first, sum up across the time periods within the weekday day type
  ...
  riders_day_type %>%
    ...
    ) %>% # then join with the more general avg_boardings for the full week
...
api_raw <- req_perform(request(bos_licenses_url))
```
