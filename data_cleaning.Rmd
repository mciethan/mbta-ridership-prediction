...
```{r}
downloaded_gtfs_path <- choose.dir(default = "", caption = "Select GTFS folder")
```

I am not sure whether this approach will work on non-Windows machines.
...
rt_stations_f23
```
Save the station information to file. The unique identifier per row is the combination of route_id and parent_station
...
## Calculate the number of bus routes that transfer to each RT station
...
```{r}
count_connecting_buses <- function(gtfs) {
  # the goal here is to count the number of bus routes serving each parent_station
  gtfs$stops %>% 
    ...
    summarise(connecting_bus_routes = n(), .groups='drop')
}

f23_buses <- f23_gtfs %>% count_connecting_buses()
f19_buses <- f19_gtfs %>% count_connecting_buses()
```
...
clean_bus_connections <- function(rt_stations_df, buses_df) {
  rt_stations_df %>%
    ...
    mutate(connecting_bus_routes = case_when(
      is.na(connecting_bus_routes) ~ 0, .default=connecting_bus_routes
      )) %>%
    select(route_id, parent_station, connecting_bus_routes)
}
