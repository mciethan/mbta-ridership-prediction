...
```{r}
ridership_f23 %>%
  select(day_type_id, time_period_name) %>%
  unique()
```
...
clean_ridership <- function(ridership) {
  riders_day_type <- ridership %>% 
    group_by(season, day_type_id, route_id, parent_station) %>%
    summarise( # first, sum up across time periods within the weekday day type
      across(c(total_ons, total_offs), sum), 
      num_svc_days = mean(number_service_days), # service days are the same within day types
      .groups='drop') %>% 
    mutate( # and create a binary daytype of just weekday vs weekend
  ...
  riders_fullweek <- riders_day_type %>%
    ...
    summarise( # sum up the three day types (weekday, sat, sun) with each other
  ...
  riders_day_type %>%
    ...
    summarise( # sum up the two weekend day types with each other
      across(c(total_ons, total_offs, num_svc_days), sum), 
      .groups='drop') %>%
    mutate(
      avg_boardings = total_ons/num_svc_days,
...
rt_stations_per_route <- function(gtfs) { 
  gtfs$routes %>%
    ...
    mutate(
      route_id = case_when(
        str_detect(route_id, 'Green') ~ 'Green', 
        .default=route_id)
      ) %>%
...
typical_rt_service <- function(gtfs) {
  gtfs$stop_times %>% 
    ...
    filter(route_pattern_typicality == 1) 
}
```
...
pivot_daytypes <- function(headways) {
  headways %>%
    ...
    group_by(route_id, parent_station, daytype) %>%
  ...
  ordered_stops %>%
    ...
    mutate(
      dist_prev_km = st_distance(
        ...
        st_point(c(stop_lon_prev, stop_lat_prev)) %>% 
          st_geometry() %>% st_set_crs(4326) %>% st_transform(26986)
        ) %>% as.numeric() / 1000,
      dist_next_km = st_distance(
        st_point(c(stop_lon, stop_lat)) %>% 
          st_geometry() %>% st_set_crs(4326) %>% st_transform(26986), 
        st_point(c(stop_lon_next, stop_lat_next)) %>% 
          st_geometry() %>% st_set_crs(4326) %>% st_transform(26986)
        ) %>% as.numeric() / 1000,
      avg_spacing_km = mean(c(dist_prev_km, dist_next_km), na.rm=T)
    ) %>%
    group_by(route_id, parent_station) %>%
    # this final step takes the average spacing across branches (e.g. for JFK)
