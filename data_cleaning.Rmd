...
MBTA GTFS recap link: https://www.arcgis.com/home/item.html?id=9ab1dc7ea2bf4ad7b7e25cc6b941b39a

```{r}
f23_gtfs <- read_gtfs(path.expand('~/Data/GTFS pre-rating-recaps/2023-fall-prerating-recap.zip'))
f19_gtfs <- read_gtfs(path.expand('~/Data/GTFS pre-rating-recaps/2019-fall-prerating-recap.zip'))
```
...
rt_stations_per_route <- function(gtfs) { 
  ...
  select(route_id, route_long_name, parent_station, stop_name, stop_lat, stop_lon) %>%
...
```{r}
rt_stations_f23 %>% write.csv('data/rapid_transit_stations_by_line_f23.csv', row.names=FALSE)
rt_stations_f19 %>% write.csv('data/rapid_transit_stations_by_line_f19.csv', row.names=FALSE)
```
...
rt_stations_f23 <- read.csv('data/rapid_transit_stations_by_line_f23.csv')
rt_stations_f19 <- read.csv('data/rapid_transit_stations_by_line_f19.csv')
```

Be careful about how many times you run this - the openrouteservice isochrones API limits you to 500 requests per day per API key. Each request returns 5 polygons, and we have ~150 stops, so a full run will use ~30 requests. There's also 20 request per minute throttling, so the full run will take ~1.5 minutes.
```{r}
walk_polys_f23 <- rt_stations_f23 %>%
  mutate( # API expects a df with these two col names and nothing else
    lon=stop_lon, lat=stop_lat, .keep='none'
    ) %>% 
  split(rep(1:ceiling(nrow(rt_stations_f23)/5),each=5)) %>% # split into groups of 5
  lapply(function(df) {
...
```{r}
walk_polys_f23[c('route_id', 'route_long_name', 'parent_station', 
                 'stop_name', 'stop_lat', 'stop_lon')] <- 
  rt_stations_f23[c('route_id', 'route_long_name', 'parent_station', 
                    'stop_name', 'stop_lat', 'stop_lon')]
```
...
walk_polys_f23_nonoverlap <- walk_polys_f23 %>%
...
```{r}
rt_stations_f23$walk_poly_wkt <- walk_polys_f23$geometry

rt_stations_f23 %>%
  write.csv('data/rapid_transit_stations_by_line_f23_overlapping.csv', row.names=FALSE)

walk_polys_f23_nonoverlap %>%
  mutate(walk_poly_wkt = st_as_text(geometry)) %>%
  select(-c(value, group_index, center)) %>%
  st_drop_geometry() %>%
  write.csv('data/rapid_transit_stations_by_line_f23_nonoverlapping.csv', row.names=FALSE)
```
